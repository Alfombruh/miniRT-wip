...
static t_vec	up_vector(t_rt *rt)
{
	if (rt->cam.n.x == 0 && (rt->cam.n.y == 1
			|| rt->cam.n.y == -1) && rt->cam.n.z == 0)
		return (v_new(0, 0, 1));
	else
		return (v_new(0, 1, 0));
}
...
//This function makes a matrix thas is gonna be used to...
//...transform the cam rays so we can move our cam
...
t_vec	matrix_vector(t_matrix m,	t_vec ray)
{
	...
	new.x = m.m[0][0] * ray.x + m.m[1][0]
		* ray.y + m.m[2][0] * ray.z + m.m[3][0];
	new.y = m.m[0][1] * ray.x + m.m[1][1]
		* ray.y + m.m[2][1] * ray.z + m.m[3][1];
	new.z = m.m[0][2] * ray.x + m.m[1][2]
		* ray.y + m.m[2][2] * ray.z + m.m[3][2];
	return (new);
}
